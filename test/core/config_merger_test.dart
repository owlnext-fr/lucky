...
void main() {
  group('ConfigMerger.mergeHeaders', () {
    test(
        'connector only',
        () => expect(
            ConfigMerger.mergeHeaders({'A': '1'}, null), equals({'A': '1'})));
    test(
        'request only',
        () => expect(
            ConfigMerger.mergeHeaders(null, {'B': '2'}), equals({'B': '2'})));
    test(
        'request overrides connector',
        () => expect(ConfigMerger.mergeHeaders({'A': 'old'}, {'A': 'new'}),
            equals({'A': 'new'})));
  ...
  group('ConfigMerger.mergeQuery', () {
    ...
    test(
        'request overrides connector',
        () => expect(ConfigMerger.mergeQuery({'page': '1'}, {'page': '2'}),
            equals({'page': '2'})));
    test('merges without conflict', () {
