...
void main() {
  group('ExponentialBackoffRetryPolicy.delayFor', () {
    ...
    test(
        'attempt 1 → 500ms',
        () => expect(
            policy.delayFor(1), equals(const Duration(milliseconds: 500))));

    test(
        'attempt 2 → 1000ms',
        () => expect(
            policy.delayFor(2), equals(const Duration(milliseconds: 1000))));

    test(
        'attempt 3 → 2000ms',
        () => expect(
            policy.delayFor(3), equals(const Duration(milliseconds: 2000))));
